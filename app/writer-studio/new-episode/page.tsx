...
export default async function WriterStudioNewEpisodePage() {
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="theme-meta mt-3 max-w-3xl text-sm leading-6">
          Start with a blank canvas or step back into one of your recent drafts.
          The flow is API-ready, but still intentionally lightweight while the studio
          foundation settles in.
        </p>
      </div>
      ...
      <div className="grid gap-6 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
        <div className="theme-panel rounded-[24px] border border-[var(--border-color)] p-5">
          ...
          <p className="theme-body mt-4 text-sm leading-6">
            Route directly into the editor with empty placeholder state so structure,
            pacing, and the creator tool layout can all be tested cleanly.
          </p>

          <div className="mt-5">
            <Link href="/writer-studio/editor?mode=new" className="story-button-primary">
              Start New
            </Link>
          </div>
        </div>
          ...
          <div className="mt-5 space-y-3">
            {drafts.map((draft) => (
              ...
              >
                ...
                <p className="theme-meta mt-3 text-xs">Progress {draft.completion}%</p>
              </Link>
            ))}
          </div>
        </div>
